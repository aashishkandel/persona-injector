...
describe("Injector", () => {
	...
	it("should create copilot instructions file", async () => {
		const results = await injectPersonas(["developer"], {
			cwd: testDir,
			targets: ["copilot"],
		});
		...
		expect(content).toContain("AI-PERSONAS:developer:START");
	...
	it("should create Claude rule files", async () => {
		...
		expect(
			await fileExists(join(rulesDir, "persona-developer-workflows.md")),
		).toBe(true);
	...
	it("should be idempotent — no duplicates on re-install", async () => {
		...
		const matches = content?.match(/AI-PERSONAS:developer:START/g);
	...
	it("should handle dry run without writing files", async () => {
		...
		const copilotExists = await fileExists(
			join(testDir, ".github/copilot-instructions.md"),
		);
