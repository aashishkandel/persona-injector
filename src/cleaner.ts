...
export async function removePersonas(
	...
	for (const { path, targetId } of [
		...
		const singleResults = await removeFromSingleFile(
			personaNames,
			filePath,
			targetId,
		);
	...
	for (const { path, targetId } of [
		...
		const multiResults = await removeFromMultiFile(
			personaNames,
			join(cwd, path),
			targetId,
		);
...
async function removeFromMultiFile(
	...
	const remainingPersonaFiles = remainingEntries.filter(
		(e) =>
			e.startsWith(MULTI_FILE_PERSONA_PREFIX) &&
			(e.endsWith(".md") || e.endsWith(".mdc")),
	);
...
async function cleanReadme(
	...
	if (!readmeContent?.includes(README_MARKER_START)) return;
