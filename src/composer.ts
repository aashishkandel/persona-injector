 ...
 */
export async function checkCompatibility(personaNames: string[]): Promise<CompositionWarning[]> {
...
function generateManifest(personaNames: string[]): string {
  ...
  return [
    ...
    `# Active AI Personas`,
    ``,
    `> Installed by [persona-injector](https://github.com/aashishkandel/persona-injector) v${PACKAGE_VERSION}`,
 ...
 */
function wrapPersonaForSingleFile(name: string, content: PersonaContent): string {
  const sections = [
    content.persona,
    '',
...
export async function composeForSingleFile(
  ...
  const personaSections = contents.map((c) => wrapPersonaForSingleFile(c.meta.name, c));
...
export async function composeForMultiFile(
  ...
  const manifestContent = [
    '# Persona Injector — Installed Personas',
    '',
    `> Generated by [persona-injector](https://github.com/aashishkandel/persona-injector) v${PACKAGE_VERSION}`,
