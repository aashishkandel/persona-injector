...
import { createSpinner, log, pc } from "../logger.js";
import {
	getAllPersonaNames,
...
export async function handleInstall(
	...
	if (personaNames.length === 0 && !options.all) {
		...
		const selectedTargets = await p.multiselect({
			message:
				"Which IDEs or systems do you use? (Select targets to inject into)",
	...
	const spinner = createSpinner(
		`Loading ${valid.length} persona${valid.length > 1 ? "s" : ""}...`,
	);
	spinner.start();
	...
	try {
		...
		log.success(
			`Loaded ${valid.length} persona${valid.length > 1 ? "s" : ""}: ${pc.cyan(valid.join(", "))}`,
		);
		...
		for (const [target, targetResults] of byTarget) {
			...
			for (const r of targetResults) {
				...
				const scopeInfo =
					r.target === "claude" && r.filePath
						? pc.dim(r.filePath.split("/").pop() ?? "")
						: "";
				console.log(
					`    ${statusIcon} ${scopeInfo || r.filePath} ${statusLabel}`,
				);
			}
		}
