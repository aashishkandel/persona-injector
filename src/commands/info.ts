...
export async function handleInfo(personaName: string): Promise<void> {
	try {
		...
		log.blank();
		console.log(
			`  ${pc.bold(`🎭 ${capitalize(meta.name)} Persona`)} ${pc.dim(`v${meta.version}`)}`,
		);
		console.log(`  ${pc.dim("─".repeat(50))}`);
		console.log(`  ${pc.dim("Category:")}   ${meta.category}`);
		console.log(`  ${pc.dim("Tags:")}       ${meta.tags.join(", ")}`);
		console.log(
			`  ${pc.dim("Compatible:")} ${meta.compatibleWith.join(", ") || "all"}`,
		);

		if (meta.conflictsWith.length > 0) {
			console.log(
				`  ${pc.dim("Conflicts:")}  ${pc.yellow(meta.conflictsWith.join(", "))}`,
			);
		}

		if (meta.fileScopes.claude) {
			console.log(
				`  ${pc.dim("File scope:")} ${meta.fileScopes.claude.join(", ")}`,
			);
		}
